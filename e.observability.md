...
```bash
kubectl run nginx --image=nginx --restart=Never --generator=run-pod/v1 --dry-run -o yaml > pod.yaml
vi pod.yaml
```
...
```bash
kubectl run nginx --image=nginx --restart=Never --generator=run-pod/v1 --port=80 --dry-run -o yaml > pod.yaml
vi pod.yaml
```
...
```bash
kubectl run busybox --image=busybox --restart=Never --generator=run-pod/v1 -- /bin/sh -c 'i=0; while true; do echo "$i: $(date)"; i=$((i+1)); sleep 1; done'
...
```bash
kubectl run busybox --image=busybox --restart=Never --generator=run-pod/v1 -- /bin/sh -c 'ls /notexist'
...
```bash
kubectl run busybox --image=busybox --generator=run-pod/v1 --restart=Never -- notexist
