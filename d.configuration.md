...
```bash
kubectl run nginx --image=nginx --restart=Never --generator=run-pod/v1 --requests='cpu=100m,memory=256Mi' --limits='cpu=200m,memory=512Mi'
```
...
```bash
kubectl run nginx --image=nginx --restart=Never --generator=run-pod/v1 --serviceaccount=myuser -o yaml --dry-run > pod.yaml
